...
const BatchJobActivityCard = (props: { batchJob: BatchJob }) => {
    ...
    const onDeleteFile = async () => {
        ...
        try {
            ...
            notification(
                'Success',
                `${operation} file has been removed`,
                'success'
            );
        } catch (e) {
            notification(
        ...
        const icon =
            ...
            batchJob.status !== 'canceled' ? (
                ...
                <FileIcon
                    className={clsx({
                        'text-grey-40':
                            Math.abs(relativeTimeElapsed.raw) >
                            twentyfourHoursInMs
                    })}
                    size={20}
                />
        ...
        const buildButton = (
            ...
            return (
                <Button
                    ...
                    size={'small'}
                    className={clsx(
                        'inter-small-regular flex justify-start',
                        className
                    )}
                    variant={variant}
                >
        ...
        return (
            (canDownload || canCancel) && (
                <div className="mt-6 flex">
                    ...
                    {canCancel &&
                        buildButton(() => cancelBatchJob(), 'danger', 'Cancel')}
                </div>
            )
        );
    };
