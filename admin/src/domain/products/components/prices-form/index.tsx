...
const PricesForm = ({ form }: Props) => {
    ...
    const priceObj = useMemo(() => {
        ...
        const currencyPrices = fields.filter(
            (field) => field.region_id === null
        );
        ...
        currencyPrices.forEach((price) => {
            obj[price.currency_code!] = {
                ...
                regionPrices: regionPrices
                    .filter(
                        (regionPrice) =>
                            regionPrice.currency_code === price.currency_code
                    )
                    .map((rp) => ({
                        ...rp,
                        regionName:
                            regions?.find((r) => r.id === rp.region_id)?.name ||
                            '',
