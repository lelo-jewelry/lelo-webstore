...
const COLUMNS: Column<PublishableApiKey>[] = [
    ...
    },
    {
        accessor: 'id',
        Header: (
            <div className="text-small font-semibold text-gray-500">Token</div>
        ),
        Cell: ({ row: { original } }) => {
                    ...
                    {table.rows.map((row) => {
                        ...
                        return (
                            <PublishableKeyTableRow
                                ...
                                row={row}
                                showDetails={() =>
                                    props.showDetailsModal(row.original)
                                }
                                showChannelsModal={() =>
                                    props.showChannelsModal(row.original)
                                }
            ...
            {!keys?.length && !isLoading && (
                <div className="flex h-[480px] w-[100%] items-center justify-center">
                    <span className="text-gray-400">
                        No keys yet, use the above button to create your first
                        publishable key
                    </span>
                </div>
            )}
