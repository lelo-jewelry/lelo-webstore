    ...
    <div className="inter-small-regular flex text-grey-50 ">
        ...
        <span className="ml-3 flex items-center">
            ...
            <span className="ml-2.5">
                {moment.utc(date).format('UTC HH:mm')}
            </span>
        </span>
    </div>
);
...
const useDiscountConfigurations = (discount: Discount) => {
    ...
    conditions.push({
        title: 'Start date',
        description: (
            <DisplaySettingsDateDescription date={discount.starts_at} />
        )
    });
    ...
    if (discount.ends_at) {
        conditions.push({
            title: 'End date',
            description: (
                <DisplaySettingsDateDescription date={discount.ends_at} />
            ),
            actions: [
                {
    ...
    if (discount.usage_limit) {
        conditions.push({
            ...
            description: (
                <CommonDescription
                    text={discount.usage_limit.toLocaleString('en')}
                />
            ),
            actions: [
                {
